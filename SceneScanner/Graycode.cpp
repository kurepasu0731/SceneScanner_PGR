...
void GRAYCODE::makeCorrespondence()
{
	...
	code_restore();
	//	補完処理on off
	interpolation(); //3次元復元の時は補完したほうがよいのでは？
}
...
void GRAYCODE::transport_camera_projector(cv::Mat &src, cv::Mat &dst)
{
	...
	for( int y = 0; y < PRJ_HEIGHT; y++ ) {
		for( int x = 0; x < PRJ_WIDTH; x++ ) {
			...
			if( p.x != -1 ) {
				//if(src_resize.at<uchar>( p.y, p.x ) != 0){ //いらなくね？
				//if(src_resize.at<uchar>( p.y, 3*p.x ) != 0 && src_resize.at<uchar>( p.y, 3*p.x+1 ) != 0 && src_resize.at<uchar>( p.y, 3*p.x+2 ) != 0){ //いらなくね？
